...
elseif ($action === 'get_dashboard_data') {
    ...
    // Added profile_pic to selection
    ...
    $data = [
        ...
        'profile_pic' => $userRes['profile_pic'], // Send pic to dashboard
        'rank' => $rank,
        'donations' => $count,
        'lives_saved' => $count * 3,
...
elseif ($action === 'book_appointment') {
    ...
    $check->execute();
    if ($check->get_result()->num_rows > 0) { sendJson('error', 'Sorry! This slot was just taken.'); }
    try {
        ...
        if ($stmt->execute()) { sendJson('success', 'Appointment requested! Wait for admin approval.'); } 
        else { sendJson('error', 'Booking failed.'); }
    } catch (Exception $e) { sendJson('error', 'Slot already taken.'); }
}

// =============================================================
// 4. PROFILE MANAGEMENT (WITH FILE UPLOAD)
// =============================================================
...
elseif ($action === 'update_profile') {
    ...
    if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_ext, $allowed)) {
            ...
            sendJson('error', 'Invalid file type. Only JPG, PNG, GIF allowed.');
        }
    }
