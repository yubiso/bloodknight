...
elseif ($action === 'get_all_blood_reports') {
    $sql = "SELECT br.report_id, br.report_date, br.hemoglobin, br.hematocrit, br.platelet_count,
                   ...
                   COALESCE(bd.location_name, 'N/A') as location_name,
    ...
    sendJson('success', 'All blood reports loaded', $reports);
}
...
elseif ($action === 'create_blood_report') {
    ...
    $stmt = $conn->prepare("INSERT INTO blood_report (user_id, appt_id, report_date, hemoglobin, hematocrit, platelet_count, white_blood_cell_count, red_blood_cell_count, blood_pressure, temperature, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
    // For nullable values in MySQLi, we pass NULL directly and use appropriate type
    $stmt->bind_param("iisddidddds", $user_id, $appt_id, $report_date, $hemoglobin, $hematocrit, $platelet_count, $wbc_count, $rbc_count, $blood_pressure, $temperature, $notes);
