...
elseif ($action === 'get_stats') {
    try {
        ...
        if (!isset($conn) || $conn === null || $conn->connect_error) {
            sendJson('error', 'Database connection failed', [
                'donors' => 0, 'pending_appt' => 0, 'confirmed_appt' => 0,
                'total_donations' => 0, 'volume_l' => '0.0',
                'confirmed_roster' => [], 'donation_history' => []
            ]);
        }
        ...
        $donorsStmt = $conn->prepare("SELECT COUNT(DISTINCT u.user_id) as c 
                                      ...
                                      WHERE d.hospital_id = ?");
        if (!$donorsStmt) {
            error_log("Failed to prepare donors query: " . $conn->error);
            $donors = 0;
        } else {
            $donorsStmt->bind_param("i", $hospital_id);
            if (!$donorsStmt->execute()) {
                error_log("Failed to execute donors query: " . $donorsStmt->error);
                $donors = 0;
            } else {
                $result = $donorsStmt->get_result();
        ...
        $pendingStmt = $conn->prepare("SELECT COUNT(*) as c 
                                       ...
                                       WHERE d.hospital_id = ? AND a.status='Pending'");
        if (!$pendingStmt) {
            error_log("Failed to prepare pending query: " . $conn->error);
            $pending = 0;
        } else {
            $pendingStmt->bind_param("i", $hospital_id);
            if (!$pendingStmt->execute()) {
                error_log("Failed to execute pending query: " . $pendingStmt->error);
                $pending = 0;
            } else {
        ...
        $volumeStmt = $conn->prepare("SELECT SUM(COALESCE(br.volume_ml, a.volume_ml)) as v 
                                      ...
                                      AND (br.volume_ml IS NOT NULL OR a.volume_ml IS NOT NULL)");
        if (!$volumeStmt) {
            error_log("Failed to prepare volume query: " . $conn->error);
            $volume = 0;
        } else {
            $volumeStmt->bind_param("i", $hospital_id);
            if (!$volumeStmt->execute()) {
                error_log("Failed to execute volume query: " . $volumeStmt->error);
                $volume = 0;
            } else {
                $result = $volumeStmt->get_result();
                $row = $result->fetch_assoc();
                $volume = $row['v'] ?? 0;
            }
            $volumeStmt->close();
        ...
        $confirmedStmt = $conn->prepare("SELECT COUNT(*) as c 
                                         ...
                                         WHERE d.hospital_id = ? AND a.status='Confirmed'");
        if (!$confirmedStmt) {
            error_log("Failed to prepare confirmed query: " . $conn->error);
            $confirmed_appt = 0;
        } else {
            $confirmedStmt->bind_param("i", $hospital_id);
            if (!$confirmedStmt->execute()) {
                error_log("Failed to execute confirmed query: " . $confirmedStmt->error);
                $confirmed_appt = 0;
            } else {
                $result = $confirmedStmt->get_result();
                $row = $result->fetch_assoc();
                $confirmed_appt = $row['c'] ?? 0;
            }
            $confirmedStmt->close();
        }
        ...
        if (!$totalDonationsStmt) {
            error_log("Failed to prepare total donations query: " . $conn->error);
            $total_donations = 0;
        } else {
            $totalDonationsStmt->bind_param("i", $hospital_id);
            if (!$totalDonationsStmt->execute()) {
                error_log("Failed to execute total donations query: " . $totalDonationsStmt->error);
                $total_donations = 0;
            } else {
                $result = $totalDonationsStmt->get_result();
                $row = $result->fetch_assoc();
                $total_donations = $row['c'] ?? 0;
            }
        ...
        $rosterStmt = $conn->prepare("SELECT u.user_id, u.full_name, d.location_name, d.drive_date, a.selected_time 
                                      ...
                                      LIMIT 5");
        if (!$rosterStmt) {
            error_log("Failed to prepare roster query: " . $conn->error);
            $confirmed_roster = [];
        } else {
            $rosterStmt->bind_param("i", $hospital_id);
            if (!$rosterStmt->execute()) {
                error_log("Failed to execute roster query: " . $rosterStmt->error);
                $confirmed_roster = [];
            } else {
        ...
        if (!$historyStmt) {
            ...
            if (!$historyStmt->execute()) {
                error_log("Failed to execute history query: " . $historyStmt->error);
                $donation_history = [];
            } else {
...
elseif ($action === 'get_all_blood_reports') {
    try {
        ...
        // Exclude reports without hospital location
        // Note: Removed br.created_at from ORDER BY as it might not exist in all schemas
        $stmt = $conn->prepare("SELECT br.report_id, br.report_date, br.hemoglobin, br.hematocrit, br.platelet_count,
                                       ...
                                       br.temperature, br.notes, br.user_id, br.appt_id,
                                ...
                                INNER JOIN appointment a ON br.appt_id = a.appt_id
                                INNER JOIN blood_drive bd ON a.drive_id = bd.drive_id
                                INNER JOIN hospital h ON bd.hospital_id = h.hospital_id
                                WHERE bd.hospital_id = ?
                                ORDER BY br.report_date DESC");
        
        if (!$stmt) {
            error_log("Failed to prepare get_all_blood_reports query: " . $conn->error);
            sendJson('error', 'Failed to prepare query', []);
        }
        ...
        if (!$stmt->execute()) {
            error_log("Failed to execute get_all_blood_reports query: " . $stmt->error);
            sendJson('error', 'Failed to execute query', []);
        }
        ...
        error_log("Error in get_all_blood_reports: " . $e->getMessage());
        sendJson('error', 'Server error: ' . $e->getMessage(), []);
    }
}
...
elseif ($action === 'forgot_password') {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $resetLink = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/reset_password.html?token=" . $token . "&user_type=hospital";
...
elseif ($action === 'reset_password') {
    ...
    $stmt = $conn->prepare("SELECT hospital_id, admin_email, hospital_name, admin_name, status FROM hospital WHERE reset_token = ? AND reset_token_expiry > NOW()");
